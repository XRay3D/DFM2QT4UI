...
CUiDomElement::CUiDomElement(const QDomElement& x)
    : QDomElement(x) { }
...
int CUiDomElement::setUiPropertyGeometry(const QRect& aGeo) {
    return (setUiPropertyGeometry(QString::number(aGeo.x()),
        QString::number(aGeo.y()),
        QString::number(aGeo.width()),
        QString::number(aGeo.height())));
}
 ...
 * @return 0 on Success and -1 if it failed.
 **/
int CUiDomElement::setUiPropertyGeometry(const QString& x,
    const QString& y,
    const QString& w,
    const QString& h) {
    QDomDocument domDoc = this->ownerDocument();
    ...
    // set values
    if(0 != rectElm.setUiDomKeyValuePair("x", x) || 0 != rectElm.setUiDomKeyValuePair("y", y)
...
int CUiDomElement::addUiGlobalAction(const QString& aName,
    const QString& aText,
...
QString CUiDomElement::getRandName(QString aPrefix, QString aSuffix) {
    ...
    randName += QString("%1").arg(number, 5, 10, '0');
