...
void writeLines(XMLProperties prop, string propname, string objectname)
{
    ...
    case objectopen:
        ...
        break;
    case propertyopen:
        iter = xmlopenmmap.find("property");
        if (iter != xmlopenmmap.end()) {
            str = iter->second;
        ...
        break;
    case attributeopen:
        iter = xmlopenmmap.find("attribute");
        if (iter != xmlopenmmap.end()) {
            str = iter->second;
            str.replace(str.find("*", 0), 1, propname, 0, str.length());
            ofile << str << endl;
    ...
    case layoutopen:
        break;
    case asline:
        iter = xmlopenmmap.find(propname);
        str = iter->second;
...
void writeObjectProperty(multimap<string, string> map)
{
    ...
    multimap<string, string>::iterator iter;
    if (!map.empty()) {
        if ((iter = map.find("x")) != map.end() && (iter = map.find("y")) != map.end()
        ...
        if ((iter = map.find("bold")) != map.end() || (iter = map.find("weight")) != map.end()
        ...
        if ((iter = map.find("addaction")) != map.end()) {
            map.erase(iter);
            objclosed++;
            if ((iter = map.find("string")) != map.end()) { //<addaction name="actionNew" />
                if (iter->second == "-")
        ...
        for (iter = map.begin(); iter != map.end(); iter++) {
            if (iter->first == "widget") {
                ...
                ofile << "<" << iter->first << ">" << iter->second << "</" << iter->first << ">"
                      << endl;
            }
        }
    }
        ...
        if (loc != string::npos && loco != string::npos) {
            ...
            if (loc != string::npos && pan == 0 && str.size() == 3) //&& parseobject
            {
                ...
                if (loc == string::npos && pan == 0 && parseobject) {
                    ...
                    multimap<string, string>::iterator iter = propertiesmmap.find(
                        getPropertyName(str));
                    if (iter != propertiesmmap.end()) {
                        ...
                        if ((ssiter = eventsmmap.find(iter->second)) != eventsmmap.end()) {
                            sendersignalmmap.insert(
                                pair<string, string>(objnamesave.top(), ssiter->second));
                        ...
                        if (iter->second == "pointsize") {
                            ...
                            if (ismenu) {
                                ...
                                if (ismenuitem) {
                                    if (menucount > 1) {
                                        saveobjectsmmap.insert(
                                            pair<string, string>("addaction",
                                                                 getPropertyValue(str)));
                                    }
                                    saveobjectsmmap.insert(
                                        pair<string, string>(iter->second, getPropertyValue(str)));
                                } else
                                    saveobjectsmmap.insert(
                                        pair<string, string>(iter->second, getPropertyValue(str)));
                            }
